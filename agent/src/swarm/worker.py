...
def build_worker_prompt(
    ...
    prompt_parts.append(
        ...
        "**Phase 2 — Data Acquisition (MANDATORY, ≤5 tool calls):**\n"
        "You MUST fetch real data before writing any analysis. This is not optional.\n"
        "- `load_skill` first to get data access methods and API patterns.\n"
        "- Write a Python script via `write_file` that fetches the data you need (prices, financials, "
        "technical indicators), then run it with `bash python script.py`.\n"
        "- For A-shares: use tushare (e.g. `pro.daily_basic`, `pro.income`, `pro.fina_indicator`).\n"
        "- For HK/US stocks: use yfinance (e.g. `yf.Ticker('01772.HK').info`, `.financials`, `.history()`).\n"
        "- For crypto: use ccxt or the OKX API patterns from load_skill.\n"
        "- Do NOT proceed to Phase 3 until you have real data in hand.\n\n"
        "**Phase 3 — Analysis & Report (≤10 tool calls):**\n"
        "- Analyze the REAL data you fetched. Compare with the Ground Truth block if present.\n"
        "- Every specific number in your report MUST come from a tool call result, NOT from memory.\n"
        ...
        "**Phase 4 — Summarize (MUST use write_file):**\n"
